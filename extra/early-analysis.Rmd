...
Next, we are going to manually create a 'model' to attempt to predict the glitch type based on these variables, as well as `ifo`, `amplitude`, `phase`, and `central_frequency`. Unlike an actual model, we will not be predicting all values simultaneously, but will have a more complex elimination approach to the data, which we will create manually.
...
```{r snr-split}
gravity_spy_predictions <- gravity_spy_metadata %>%
  filter(!(label %in% c("Repeating_Blips", "Light_Modulation"))) %>%
...
Next, we attempt to select the second population of `Extremely_Loud` glitches by noting that they have durations greater than 4s, peak frequencies between 18 and 55Hz, and SNRs over 30.